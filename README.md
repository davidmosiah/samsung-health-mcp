...
- `SAMSUNG_HEALTH_PRIVACY_MODE` defaults to `summary`; raw record dumps are opt-in. It applies to every data path: `samsung_health_list_records`, `samsung_health_list_workouts` and `samsung_health_daily_summary` (which returns individual workout records only when `privacy_mode` is `structured` or `raw`). `samsung_health_weekly_summary` never returns individual workout records.
